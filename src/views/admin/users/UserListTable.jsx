  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('fullName', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            {getAvatar({ avatar: row.original.avatar, fullName: row.original.fullName || row.original.name })}
            <div className='flex flex-col'>
              <Typography color='text.primary' className='font-medium'>
                {row.original.fullName || row.original.name || 'N/A'}
              </Typography>
      ...
      columnHelper.accessor('role', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-2'>
            <Icon
              className={classnames('text-[22px]', userRoleObj[row.original.role]?.icon || userRoleObj.user.icon)}
              sx={{ color: `var(--mui-palette-${userRoleObj[row.original.role]?.color || userRoleObj.user.color}-main)` }}
            />
      ...
      columnHelper.accessor('currentPlan', {
        ...
        cell: ({ row }) => (
          <Typography className='capitalize' color='text.primary'>
            {row.original.currentPlan || row.original.subscription?.plan || 'free'}
          </Typography>
        )
      }),
